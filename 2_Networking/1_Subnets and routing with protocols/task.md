...
For each IP address `120.156.21.95/12`, `172.18.54.141/19`, `192.167.43.58/27`:
...
For the network shown in Figure 1, the address `172.16.0.0/20` is assigned.
...
2.1. Divide the network into subnets based on the number of hosts in each of the subnets and using a fixed length subnet mask (FLSM).

2.2. Calculate how many addresses are left unused in each of the subnets.

2.3. **Optional extra task:** perform steps 2.1 to 2.2 using a variable length subnet mask (VLSM).
